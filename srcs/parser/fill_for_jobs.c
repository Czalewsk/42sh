...
int		execute_pipe_run(t_tree *c, t_job *job, t_process *c_p)
{
	t_tree	*tmp;
	int			i;
...
//	termcaps_restore_tty();
//	set_fd(c_p);
	if (check_built_in(c_p) == 0)
		do_built_in(c_p);
	else
		execute(c_p, job, env_make(ENV_GLOBAL | ENV_TEMP), 0);
//	reset_fdd(c_p);
//	termcaps_set_tty();
	return (c_p->returned);
}
...
t_tree			*new_success_or_if(t_tree *c)
{
	while (c)
	{
		if (c->token.id == PIPE || c->token.id == AND_IF)
			return (c);
		c = c->right;
	}
	return (c);
...
t_tree 			*split_and_if_pipe(t_tree *c, t_job *job)
{
	...
	ft_printf("SPLIT_AND_IF_PIPE IN %s\n", tmp->token.str);
